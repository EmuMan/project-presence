...
public class JumpingEnemy : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (math.sqrt(math.pow(math.abs(playerTransform.position.x - transform.position.x), 2) + math.pow(math.abs(playerTransform.position.z - transform.position.z), 2)) <= jumpHorizontalAmount)
        {
            jumpHorizontalAmount = jumpHorizontalAmount / 2;
        }

        else if (math.sqrt(math.pow(math.abs(playerTransform.position.x - transform.position.x), 2) + math.pow(math.abs(playerTransform.position.z - transform.position.z), 2)) >= jumpHorizontalAmount)
        {
            jumpHorizontalAmount = jumpHorizontalAmount * 2;
        }
    ...
    void JumpTowardPlayer(Transform playerTransform)
    {
