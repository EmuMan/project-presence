...
public class PlayerModules : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        headModule?.PerformActionIfAvailable(headInputTracker.IsPressed(), deltaTime);
        coreModule?.PerformActionIfAvailable(coreInputTracker.IsPressed(), deltaTime);
        leftArmModule?.PerformActionIfAvailable(leftArmInputTracker.IsPressed(), deltaTime);
        rightArmModule?.PerformActionIfAvailable(rightArmInputTracker.IsPressed(), deltaTime);
        movementModule?.PerformActionIfAvailable(movementInputTracker.IsPressed(), deltaTime);
    }
}
